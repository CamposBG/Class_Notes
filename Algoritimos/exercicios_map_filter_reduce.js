...
const impar = randomNumbers.filter(items => items %2 !== 0)
console.log(impar)
...
const lessthan501 = crazyNumbers.reduce((accumulator, number) =>{
    if (number < 501){
        accumulator += 1
    }
    return accumulator
...
const numbers = [5, 7, 3]
const square = numbers.map(items => items**2 )
console.log(square)
console.log(numbers)
...
const moveiesBefore2000 = tarantinoMovies.filter(obj => obj["release"] < 2000)
console.log(moveiesBefore2000)
...
const tvShowsNames = tvShows.map((show) => {
    return show.name
})
...
const ganeNames = cart.reduce((acc, gameObj) =>{
   acc += `- ${gameObj["name"]}\n` 
  return acc
},"")
console.log(ganeNames)
/*
  - Nome 1
  - Nome 2
  - Nome 3
